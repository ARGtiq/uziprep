...
export function ScenarioComparisonView({ scenarios }: Props) {
  ...
  const commonStepTexts = new Set(stepsCmp.common.map((s) => s.text));
  const commonChecklistTexts = new Set(checklistCmp.common.flatMap((b) => b.items));
  ...
  return (
    <div>
      ...
      <button
        ...
        className="mb-4 flex items-center gap-1.5 text-xs font-semibold text-on-surface-variant"
      >
        <Icon name={layout === 'split' ? 'grid_view' : 'compare'} size={14} />
        {layout === 'split' ? 'В один список' : 'По группам (общее/отличия)'}
      </button>
          ...
          {scenarios.map((sc) => (
            <details key={sc.name} className="rounded-m3-md bg-surface-container-low open:pb-2">
              ...
              <div className="px-3.5">
                {mode === 'algo'
                  ? sc.stepBlocks.flatMap((block) => block.items).map((step) => {
                      const isUnique = !commonStepTexts.has(step.text);
                      return (
                        <div
                          key={step.num}
                          className={`flex gap-2.5 border-t border-outline-variant py-2.5 first:border-none ${
                            isUnique ? 'rounded-m3-sm bg-secondary-container px-2 text-on-secondary-container' : ''
                          }`}
                        >
                          <span className="w-6 shrink-0 text-xs opacity-70">{step.num}</span>
                          <p className="text-sm leading-relaxed">{step.text}</p>
                        </div>
                      );
                    })
                  : sc.checklist.map((block) => (
                      <div key={block.block} className="py-2 first:pt-0">
                        ...
                        {block.items.map((item, i) => {
                          const isUnique = !commonChecklistTexts.has(item);
                          return (
                            <div
                              key={i}
                              className={`flex gap-2.5 border-t border-outline-variant py-2 first:border-none ${
                                isUnique ? 'rounded-m3-sm bg-secondary-container px-2 text-on-secondary-container' : ''
                              }`}
                            >
              ...
              <p className="mt-2 px-3.5 text-[11px] text-on-surface-variant">
                <span className="mr-1 inline-block h-2 w-2 rounded-sm bg-secondary-container align-middle" /> — отличается от других сценариев, остальное общее
              </p>
            </details>
          ))}
        </div>
